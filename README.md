...
> Статус: **Фаза 2 завершена.** Готовы: ERC-721 `Place` (категории, метаданные, потолок) +
> seed-скрипт Воронежа; ERC-20 `CityToken` ($CITY). Оба контракта на едином `AccessControl`
> с ролью `MINTER`. Фронта пока нет.
