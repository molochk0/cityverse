...
> Статус: **Фаза 2 — токен `$CITY`.** Готовы: ERC-721 `Place` (категории, метаданные, потолок) +
> seed-скрипт Воронежа; ERC-20 `CityToken` ($CITY) с `AccessControl` и ролью `MINTER`. Фронта пока нет.
