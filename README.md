...
> Статус: **Фаза 1 — `Place` NFT + сидинг.** Реализован ERC-721 `Place` (категории, метаданные,
> потолок предложения) с тестами и deploy/seed-скрипт для Воронежа (парки реальные, кальянные —
> плейсхолдеры). Фронта пока нет.
